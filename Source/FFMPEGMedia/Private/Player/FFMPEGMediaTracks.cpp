...
TArray<const AVCodec*>  FFFMPEGMediaTracks::FindDecoders(int codecId, bool hwaccell) {
    ...
    if (hwaccell) {
        ...
        for (const AVCodec* codec12 : tmp) {
            if (isHwAccel(codec12)) {
                codecs.Add(codec12);
            }
        }
    }
    ...
    if (candidates.Num() > 0) {
        if (!hwaccell) {
            for (const AVCodec* codec11 : candidates) {
                if (!isHwAccel(codec11)) {
                    codecs.Add(codec11);
                }
            }
        }
...
int FFFMPEGMediaTracks::UploadTexture(FFMPEGFrame* vp, AVFrame *frame, struct SwsContext **img_convert_ctx) {
    ...
    *img_convert_ctx = sws_getCachedContext(*img_convert_ctx,
    ...
    FTimespan Duration12 = FTimespan::FromSeconds(vp->GetDuration());
                ...
                if (subpq.GetNumRemaining() > 0) {
                    ...
                    if (vp->GetPts() >= sp->GetPts() + ((float)sp->GetSub().start_display_time / 1000)) {
                        if (!sp->IsUploaded()) {
                            ...
                            FTimespan Duration22 = FTimespan::FromSeconds(sp->GetDuration());
                            ...
                            for (i = 0; i < (int)sp->GetSub().num_rects; i++) {
                                ...
                                if ( sub_rect->type == SUBTITLE_TEXT ||  sub_rect->type == SUBTITLE_ASS) {
                                    ...
                                    if (CaptionSample->Initialize(sub_rect->type == SUBTITLE_TEXT?sub_rect->text:sub_rect->ass, FVector2D(sub_rect->x, sub_rect->y), Time, Duration22))
                                    {
...
int FFFMPEGMediaTracks::AudioDecodeFrame(FTimespan& Time, FTimespan& Durations) {
    ...
    Durations = FTimespan::FromSeconds(af->GetDuration());
...
void FFFMPEGMediaTracks::RenderAudio() {
    ...
    FTimespan Time = 0;
    FTimespan Durations = 0;

    audio_size = AudioDecodeFrame(Time, Durations);
    if (audio_size < 0) {
    ...
    if (CurrentState == EMediaState::Paused || CurrentState == EMediaState::Stopped) {
        ...
        if ( audioBuf != NULL ) {
            ...
            if (AudioSample->Initialize((uint8_t *)audioBuf, len1, targetAudio.NumChannels, targetAudio.SampleRate, Time, Durations))
            {
